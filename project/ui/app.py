...
class App(ctk.CTk):
    ...
    def _on_data_read_to_memory(self) -> None:
        ...
        try:
            ...
            msg = (
                f"Данные в памяти: матрица {shape[0]}×{shape[1]} "
                f"(трассы {start}:{end}:{step})."
            )
            self.label_data_result.configure(text=msg, text_color=C.STATUS_OK)
            self._update_home_before_from_matrix(self.matrix_data)
    ...
    def _update_home_before_from_matrix(self, matrix: Any) -> None:
        """Обновить график «До» по полной матрице (после «Прочитать в память»)."""
    ...
    def _update_home_plots_after_load(self, preview: Optional[SeismicPreview]) -> None:
        ...
        if preview is None:
            ...
            arr = np.frombuffer(preview.data, dtype=np.float32).reshape(preview.n_traces, preview.n_samples)
            axb.imshow(
                arr.T,
                aspect="auto",
                cmap="gray",
