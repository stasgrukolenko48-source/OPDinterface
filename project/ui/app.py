...
class App(ctk.CTk):
    def __init__(self) -> None:
        ...
        self._warmup_ribbon_panels()
        self._warmup_tab_frames_layout()
    ...
    def _warmup_ribbon_panels(self) -> None:
        """Прогрев отрисовки обеих панелей ленты (они уже смонтированы через place, только поднимаем слой)."""
        ...
        self.update_idletasks()
        self.home_tools.update_idletasks()

        self.analysis_tools.tkraise()
        self.ribbon.configure(height=C.RIBBON_HEIGHT_ANALYSIS)
        self.update_idletasks()
        self.analysis_tools.update_idletasks()
    ...
    def _warmup_tab_frames_layout(self) -> None:
        """Поднять каждый фрейм вкладки и обработать очередь геометрии — контент не выгружается, только прогрев."""
        current = self.current_state["tab"]
        for name in self.tabs_list:
            self.frames[name].tkraise()
            self.update_idletasks()
        self.frames[current].tkraise()
    ...
    def setup_ribbon_tools(self) -> None:
        ...
        left_col.pack(side="left", fill="y", padx=(20, 12), anchor="nw")
        ...
        self.btn_processing.pack(anchor="w", pady=(18, 6))
